...
// a backtest, or supplemented with previous-season form), so it can be replayed.
export function buildStrengthRows(matches: readonly AflMatch[]): TeamStrengthRow[] {
...
// Predict a single match's scores from prebuilt strength rows. Pure — the backtest
// passes a teamMap built from data available before the match's round.
export function predictMatch(
  m: AflMatch,
  teamMap: Map<number, TeamStrengthRow>,
  venueAdjusted: boolean,
): UpcomingMatchPrediction {
  const home = teamMap.get(m.homeTeamId)
  const away = teamMap.get(m.awayTeamId)
  const venue = venueAdjusted
...
export function useScorePredictor(
  ...
  const strengthRows = computed<TeamStrengthRow[]>(() => buildStrengthRows(matchesRef.value))
  ...
  function buildPrediction(m: AflMatch): UpcomingMatchPrediction {
    return predictMatch(m, teamMap.value, venueAdjusted.value)
  }
