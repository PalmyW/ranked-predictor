...
import { pricing, fmtUsd } from '@/constants/pricing.js'
...
function sessionCost(stats) {
  ...
  return (
    (stats.inputTokens * p.input + stats.outputTokens * p.output) / 1_000_000
  )
}
    ...
    <div class="app-shell">
      <header class="app-header">
        <v-toolbar flat density="compact" class="app-toolbar">
          <v-toolbar-title>
            <span class="text-h6 font-weight-bold">PalmyDatabase</span>
          </v-toolbar-title>
          ...
          <div v-if="dbStatus" class="d-flex align-center ga-3 mr-4">
            <v-chip
              ...
              >{{ dbStatus.files_tracked.toLocaleString() }} files</v-chip
            >
            <span class="text-caption text-medium-emphasis d-none d-sm-inline"
              >Last import: {{ formatImportTime(dbStatus.last_import) }}</span
            >
          </div>

          <!-- Claude usage -->
          <v-menu
            v-if="aiStats?.enabled"
            location="bottom end"
            :open-on-hover="true"
            :close-delay="150"
            :open-delay="100"
          >
            <template #activator="{ props }">
              ...
              >
                ...
                >
                  {{ aiStats.requestCount }} req ·
                  {{
                    fmtTokens(aiStats.inputTokens + aiStats.outputTokens)
                  }}
                  tok
                </v-chip>
                <v-icon
                  ...
                  :color="
                    rateLimitColor(
                      aiStats.rateLimit.tokensRemaining,
                      aiStats.rateLimit.tokensLimit,
                    )
                  "
                  >mdi-circle</v-icon
                >
              ...
              <v-card-text class="pa-3">
                ...
                <div class="d-flex justify-space-between text-body-2 mb-3">
                  <span style="color: rgba(255, 255, 255, 0.6)"
                    >Session cost</span
                  >
                  <span style="color: #ecc94b; font-weight: 600">{{
                    fmtUsd(sessionCost(aiStats))
                  }}</span>
                </div>
                ...
                <template v-if="aiStats.rateLimit?.tokensLimit">
                  ...
                  <div class="d-flex justify-space-between text-body-2 mb-1">
                    ...
                    >
                    <span
                      :style="`color: ${rateLimitHex(aiStats.rateLimit.tokensRemaining, aiStats.rateLimit.tokensLimit)}; font-weight: 600`"
                    >
                      {{ fmtTokens(aiStats.rateLimit.tokensRemaining) }} /
                      {{ fmtTokens(aiStats.rateLimit.tokensLimit) }}
                    </span>
                  </div>
                  <div class="d-flex justify-space-between text-body-2 mb-1">
                    <span style="color: rgba(255, 255, 255, 0.6)"
                  ...
                  <div class="d-flex justify-space-between text-body-2 mb-1">
                    <span style="color: rgba(255, 255, 255, 0.6)"
                      >Requests remaining</span
                    >
                    <span style="color: rgba(255, 255, 255, 0.9)"
                      >{{ aiStats.rateLimit.requestsRemaining }} /
                      {{ aiStats.rateLimit.requestsLimit }}</span
                    >
                  </div>
                  <div
                    v-if="aiStats.rateLimit.tokensReset"
                    class="d-flex justify-space-between text-body-2"
                  >
                    <span style="color: rgba(255, 255, 255, 0.6)">Resets</span>
                    <span style="color: rgba(255, 255, 255, 0.9)">{{
                      new Date(
                        aiStats.rateLimit.tokensReset,
